...
For each cancer class $c$, a learnable prototype $\mathbf{p}_c \in \mathbb{R}^{Kd}$ is maintained. Classification logits are computed as cosine similarity between the normalized embedding and each prototype:

\begin{equation}
    \text{logit}_c = \frac{\mathbf{z} \cdot \mathbf{p}_c}{\tau}
\end{equation}
